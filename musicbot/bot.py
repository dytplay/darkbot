...
class MusicBot(discord.Client):
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "Запрешено использовать в канале (%s)" % vc.name, expire_in=30)
    ...
    async def on_ready(self):
        print('\rПодключено успешно!  DarkBOT v%s\n' % BOTVERSION)
        ...
        self.safe_print("                BotID: \n    %s/%s#%s" % (self.user.id, self.user.name, self.user.discriminator))

        owner = self._get_owner(voice=True) or self._get_owner()
        if owner and self.servers:
            self.safe_print("              Owner ID: \n    %s/%s#%s\n" % (owner.id, owner.name, owner.discriminator))

            print('Сервера на котором активен бот:')
        ...
        if self.config.bound_channels:
            ...
            print("Отсутствует привязка к текстовому каналу")
        ...
        if self.config.autojoin_channels:
            ...
            print("Голосовой канал для автоматического входа не настроен ")
            autojoin_channels = set()

        print()
        print("Опции:")

        self.safe_print("  " + self.config.command_prefix)
        print("  Громкости: %s%%" % int(self.config.default_volume * 100))
        print("  Порог пропуска: %s голосов %s%%" % (
            self.config.skips_required, self._fixg(self.config.skip_ratio_required * 100)))
        print("  Сейчас играет @mentions: " + ['Отключено', 'Активно'][self.config.now_playing_mentions])
        print("  Авто-вход: " + ['Отключено', 'Активно'][self.config.auto_summon])
        print("  Повторять плейлист: " + ['Отключено', 'Активно'][self.config.auto_playlist])
        print("  Автоматическая пауза: " + ['Отключено', 'Активно'][self.config.auto_pause])
        print("  Удалять сообщения бота: " + ['Отключено', 'Активно'][self.config.delete_messages])
        if self.config.delete_messages:
            print("    Delete Invoking: " + ['Отключено', 'Активно'][self.config.delete_invoking])
        print("  Debug Mode: " + ['Отключено', 'Активно'][self.config.debug_mode])
        print("  Кеширование песен %s" % ['Отключено', 'Активно'][self.config.save_videos])
        print()
        ...
        if self.config.autojoin_channels:
            ...
            print("Попытка автоматического подключения в голосовой канал...", flush=True)

            # waitfor + get value
            owner_vc = await self._auto_summon()

            if owner_vc:
                print("Готово", flush=True)  # TODO: Change this to "Joined server/channel"
                if self.config.auto_playlist:
                    print("Воспроизведение авто-плейлиста (Колличество песен - 3054 ")
                    await self.on_player_finished_playing(await self.get_player(owner_vc))
            else:
                print("Создатель отсутствует в голосовом канале. Авто-вход невозможен")
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "                    🔈Команды музыкального бота \n"
            helpmsg += " 🔈Создатель бота vk.com/dytplay Discord ID#8037  :man_with_turban::skin-tone-1:    \n :large_orange_diamond:!play ссылка на видео с YouTube - Добавит трек в плейлист \n :large_orange_diamond:!queue - отобразит текущий плейлист \n :large_orange_diamond:!skip - предложить пропустить песню \n :large_orange_diamond:!np - Отобразить сейчас текущую песню \n \n \n Для подключения бота в ваш канал обратитесь к создателю.  "
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "Песня добавлена в плэйлист **%s** songs to be played. Position in queue: %s"
            btext = str(listlen - drop_count)

        else:
            if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                raise exceptions.PermissionsError(
                    "Длительность песни слишком большая. Максимальная длительность 20 минут" % (info['duration'], permissions.max_song_length),
                    expire_in=30
                )
            ...
            reply_text = "В плейлист добавлена  **%s** . До вашей песни осталось: %s песен"
            btext = entry.title
        ...
        if position == 1 and player.is_stopped:
            ...
            try:
                ...
                reply_text += ' - До воспроизведение вашей песни осталось : %s'
            except:
                traceback.print_exc()
                time_until = ''
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Сейчас играет: **%s**. Добавил **%s** %s\n" % (
            ...
            return Response(
                'Музыка закончилась. Используйте {}play. чтобы добавить песню'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_clear(self, player, author):
        ...
        return Response(':put_litter_in_its_place: Очистка прошла успешно. ', delete_after=20)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError("Нечего пропускать.", expire_in=20)
        ...
        if skips_remaining <= 0:
            player.skip()  # check autopause stuff here
            return Response(
                'Ваше предложение пропуска  **{}** было принято'
                '\nГолосование закончено{}'.format(
                    player.current_entry.title,
                    ' Играет следующая песня' if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
            ...
            return Response(
                'хочет пропустить песню **{}**'
                'Для пропуска песни необходимо еще  \n**{}** {} голосов для пропуска.'.format(
                    player.current_entry.title,
                    skips_remaining,
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Сейчас играет: **%s** добавил **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
        ...
        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = '`{}.` **{}** добавил  **{}**'.format(i, item.title, item.meta['author'].name).strip()
            else:
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('Удалено  {} сообщений{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
    ...
    async def on_message(self, message):
        ...
        if message.author.id in self.blacklist and message.author.id != self.config.owner_id:
            ...
            self.safe_print("[Komanda] {0.id}/{0.name} ({1})".format(message.author, message_content))
    ...
    async def on_voice_state_update(self, before, after):
        ...
        if sum(1 for m in my_voice_channel.voice_members if m != after.server.me):
            if auto_paused and player.is_paused:
                print("[Автопауза] Воспроизведение")
                self.server_specific_data[after.server]['auto_paused'] = False
                player.resume()
        else:
            if not auto_paused and player.is_playing:
                print("[Автопауза] Пауза")
